...
describe('tractor-plugin-features: init:', () => {
    it('should create the features directory', () => {
        let config = {
            directory: './tractor'
        };
        ...
        return init(config)
        .then(() => {
            expect(fs.mkdirAsync).to.have.been.calledWith('/tractor/features');
        })
        .finally(() => {
    ...
    it('should just move on if the directory already exists', () => {
        ...
        return init(config)
        .then(() => {
            expect(tractorLogger.warn).to.have.been.calledWith('"/tractor/features" directory already exists. Moving on...');
        })
        .finally(() => {
            fs.mkdirAsync.restore();
            process.cwd.restore();
